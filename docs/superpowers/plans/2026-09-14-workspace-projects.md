...
- [ ] Add Project and Workspace.projects to the Prisma schema.
- [ ] Add a migration using Prisma's migration workflow and verify the SQL creates the table, foreign key, and index.
- [ ] Run Prisma client generation and backend typecheck.
...
- [ ] Write failing service tests for member listing, owner/admin mutation, member mutation rejection, and non-member workspace hiding.
- [ ] Write failing route tests for strict validation, response status/body, authentication, and CSRF on POST/PATCH/DELETE.
- [ ] Run phase-five tests and confirm they fail because the project module is absent.
...
- [ ] Implement the repository with Prisma projections and workspace-scoped mutation queries.
- [ ] Implement service validation and OWNER/ADMIN authorization.
- [ ] Implement controllers with Zod validation and consistent AppError mapping.
- [ ] Wire the router under /api/workspaces/:workspaceId/projects with authentication and CSRF.
- [ ] Run phase-five tests and confirm they pass.
...
- [ ] Write the failing database tests against the repository contract.
- [ ] Run them against the local PostgreSQL database.
- [ ] Keep the integration tests as permanent regression coverage for the tenant boundary.
...
- [ ] Add failing component/API tests for list rendering, create/update/delete requests, CSRF headers, and member read-only behavior.
- [ ] Implement the RTK Query endpoints and project page.
- [ ] Add the workspace Projects link.
...
- [ ] Add a clear link from the workspace shell to persisted projects without presenting mock projects as persisted data.
- [ ] Add frontend coverage for the real project navigation.
- [ ] Update the learning document with the final request flow and implementation order.
...
- [ ] Run backend tests, typecheck, build, lint, format check, and database check.
- [ ] Run frontend tests, typecheck, lint, and production build.
- [ ] Run git diff --check.
- [ ] Review the final status and confirm no unrelated files were changed.
